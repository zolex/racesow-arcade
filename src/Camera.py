...
class Camera(SimpleRect):
    ...
    def update(self, player):
        ...
        # DEFAULT vertical style reduces the lookahead when the players comes close to the ground
        # this results in a pretty snappy feel when landing on the ground
        if self.settings.camera_style_y == 'default':
        ...
        # SETTLING vertical style keeps the lookahead and later settles the camera back to the desired vertical position
        # this results in a pretty soft landing feel but has some issues when falling over different distances. eg. falling over big stairs
        elif self.settings.camera_style_y == 'settling':
            fall_lookahead_px = 0.0
